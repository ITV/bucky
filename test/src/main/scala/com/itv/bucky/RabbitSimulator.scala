...
class RabbitSimulator[B[_]](bindings: Bindings = IdentityBindings)(implicit M: Monad[B],
  ...
  def consumer(queueName: QueueName,
               ...
               prefetchCount: Int = 0): B[Unit] = monad.apply {
  ...
  def publish(publishCommand: PublishCommand): Future[ConsumeAction] = {
    ...
    if (isDefinedAt(publishCommand)) {
      ...
      consumers
        .get(queueName)
        .fold(Future.failed[ConsumeAction](new RuntimeException(s"No consumers found for $queueName!"))) { handlers =>
          val responses = handlers.map(
            h =>
              h(Delivery(
                publishCommand.body,
                ConsumerTag("ctag"),
                Envelope(deliveryTag.getAndIncrement(), false, publishCommand.exchange, publishCommand.routingKey),
      ...
      Future.failed(
        new RuntimeException(s"No queue defined for ${publishCommand.exchange} ${publishCommand.routingKey}"))
  }

  def queueNameFor(publishCommand: PublishCommand): QueueName =
    declarations
      .get(publishCommand.exchange, publishCommand.routingKey)
      .fold(bindings(publishCommand.routingKey))(identity)
