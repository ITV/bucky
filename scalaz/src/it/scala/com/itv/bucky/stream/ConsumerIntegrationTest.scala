...
class ConsumerIntegrationTest extends FunSuite with ScalaFutures with StrictLogging with Eventually {
  ...
  test(s"Can consume messages from a (pre-existing) queue") {
    ...
    withPublisherAndConsumer(AmqpClient.deliveryHandlerOf(handler, toDeliveryUnmarshaller(messageUnmarshaller))) { app =>
      handler.receivedMessages shouldBe 'empty

      val expectedMessage = randomString()
      app.publish(PublishCommand(app.exchangeName, app.routingKey, MessageProperties.persistentBasic, Payload.from(expectedMessage))).unsafePerformSyncAttempt should ===(\/-(()))
