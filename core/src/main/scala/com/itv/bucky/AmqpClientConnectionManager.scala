...
private[bucky] case class AmqpClientConnectionManager[F[_]](
  ...
  def publish(cmd: PublishCommand): F[Unit] =
    for {
      ...
      _ <- (for {
        signal <- Deferred[F, PublishResult]
        ...
        _ <- signal.get.flatMap {
          case PublishResult.Success     => F.unit
          case PublishResult.Failed      => F.raiseError[Unit](new RuntimeException("Failed to publish msg."))
          case PublishResult.Unrouteable => F.raiseError[Unit](new RuntimeException("Message published, but unrouteable as no bindings exist"))
        }
      } yield ())
...
private[bucky] object AmqpClientConnectionManager extends StrictLogging {
  ...
  def apply[F[_]](config: AmqpClientConfig,
    ...
    for {
      pendingConfirmations <- Ref.of[F, TreeMap[Long, Deferred[F, PublishResult]]](TreeMap.empty)
      _                    <- publishChannel.confirmSelect
      returnReceived       <- Ref.of[F, Boolean](false)
      confirmListener      <- F.delay(publish.PendingConfirmListener(pendingConfirmations, returnReceived))
