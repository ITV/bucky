...
object LoggingAmqpClient extends StrictLogging {
  ...
  private[bucky] def logSuccessfullPublishMessage[F[_]](charset: Charset, cmd: PublishCommand)(implicit F: Sync[F]): F[Unit] =
    F.delay(
      logger.info(
        "Successfully published message with rk:'{}', exchange:{} and message:'{}'",
  ...
  private[bucky] def logFailedPublishMessage[F[_]](t: Throwable, charset: Charset, cmd: PublishCommand)(implicit F: Sync[F]): F[Unit] =
    F.delay(
      logger.error(
        "Failed to publish message with rk:'{}', exchange:'{}' and message:'{}'",
        cmd.routingKey.value,
        cmd.exchange.value,
        new String(cmd.body.value, charset),
        t
  ...
  private[bucky] def logFailedHandler[F[_]](
      charset: Charset,
      queueName: QueueName,
  ...
  private[bucky] def logSuccessfulHandler[F[_]](charset: Charset, queueName: QueueName, delivery: Delivery, ca: ConsumeAction)(implicit
      F: Sync[F]
  ): F[Unit] = F.delay {
    logger.info(
      ...
      override def registerConsumer(
        ...
        val newHandler = (delivery: Delivery) =>
          (for {
            ...
            _ <- result.fold(
              logFailedHandler(charset, queueName, exceptionalAction, delivery, _),
