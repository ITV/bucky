...
object SuperTest {
  object StubChannel {
    def apply: Channel[IO] = new Channel[IO] {
      ...
      override def publish(cmd: PublishCommand): IO[Unit] = {
        val queues =
          bindings.find(binding => binding.exchangeName == cmd.exchange && binding.routingKey == cmd.routingKey).map(_.queueName)
        val h = handlers
  ...
  def withDefaultClient(test: AmqpClient[IO] => IO[Unit]): Unit = {
    ...
    val config                        = AmqpClientConfig("127.0.0.1", 5672, "guest", "guest", publishingTimeout = 5.seconds)
    AmqpClient.apply[IO](config, StubChannel.apply).flatMap(test).unsafeRunSync()
  }
}
