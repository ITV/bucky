...
case class AmqpClientLifecycle(config: AmqpClientConfig)(implicit executionContext: ExecutionContext) extends Lifecycle[AmqpClient[Lifecycle, Future, Throwable, Unit]] with StrictLogging {
  ...
  override def unwrap(instance: RabbitConnection): AmqpClient[Lifecycle, Future, Throwable, Unit] = new LifecycleRawAmqpClient(AmqpChannelLifecycle(instance))
}


case class AmqpChannelLifecycle(connection: RabbitConnection) extends VanillaLifecycle[RabbitChannel] with StrictLogging {
