...
object IdPublisher extends StrictLogging {
  ...
  def apply(channel: Channel, publisherWrapper: (Publisher[PublishCommand]) => Publisher[PublishCommand]): Id[Publisher[PublishCommand]] = {
    Try {
...
object IdConsumer extends StrictLogging {
  ...
  def apply(queueName: QueueName, handler: Handler[Delivery], actionOnFailure: ConsumeAction = DeadLetter, prefetchCount: Int = 0, channel: Channel)(implicit executionContext: ExecutionContext): Id[Unit] = {
