...
class FutureTimeoutPublisher(delegate: Publisher[Future, PublishCommand], timeout: FiniteDuration)(
    ec: ScheduledExecutorService)
    extends Publisher[Future, PublishCommand] {
  override def apply(cmd: PublishCommand): Future[Unit] = {
    ...
    delegate(cmd).onComplete(result => promise.complete(result))(ExecutionContext.fromExecutor(ec))
